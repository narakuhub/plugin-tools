...
-- --------------------------------------------------------------------------------
-- 3. HTTP DOWNLOAD FUNCTION
-- --------------------------------------------------------------------------------
local function DownloadRawBinary(rawUrl)
...
local function LoadRBXMFromRaw(rawData, rawUrl)
	...
	if not (isBinary or isXml or isExtensionValid) then
		return nil, "Unsupported RBXM/RBXL"
	end
	...
	if getcustomasset and writefile then
		...
		if successAsset and assetId then
			...
			if successObjects and objects then
...
local function PositionWorkspaceObject(instance)
	...
	local targetCFrame = camera and (camera.CFrame * CFrame.new(0, 0, -30)) or CFrame.new(0, 5, 0)
...
local function InsertFileFromRaw(rawUrl, statusTargetBtn)
	...
	local btnTextObj = statusTargetBtn:FindFirstChildOfClass("TextLabel") or statusTargetBtn
	...
	local rawData, errDownload = DownloadRawBinary(cleanUrl)
	if errDownload then
